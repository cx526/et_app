<template>
	<view :class="[
	 ...
	 @click="onClick()" class="uni-tag" v-if="text">
		<text :class="[type === 'default' ? 'uni-tag--default':'uni-tag-text',inverted === true || inverted === 'true' ? 'uni-tag-text--'+type : '',size === 'small' ? 'uni-tag-text--small':'',color === '' ? '' : 'color-info' ]">{{ text }}</text>
	</view>
</template>
