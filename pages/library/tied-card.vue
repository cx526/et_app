<template>
	<view>
		<view class="form">
			<view class="item">
				...
				<picker class="picker-input" mode="selector" @change="bindSchoolChange" :value="schoolIndex" :range="schoolArray" range-key="name" :disabled="isDisabled">
					<input type="text" placeholder="请选择学生所在学校" placeholder-style="font-size: 30rpx; color: #999" disabled :value="schoolName" />
				</picker>
			</view>
			<view class="item">
			...
			<view class="item">
				...
				<picker class="picker-input" @change="bindClassChange" :value="classIndex" :range="classArray" :disabled="isDisabled">
					<input type="text" placeholder="请选择学生所在班级" placeholder-style="font-size: 30rpx; color: #999" :value="className + '班'" disabled />
				</picker>
				<!-- <image
				 :src="$aliImage + 'teacher-icon-02.png'" 
				 mode="widthFix"
			...
			<view class="item">
				...
				<input type="text" placeholder="请扫码绑定童书卡" placeholder-style="font-size: 30rpx; color: #999" @input="getCardNumber" :disabled="isDisabled" v-model="card_no" />
				<image :src="$aliImage + 'teacher-icon-02.png'" mode="widthFix" style="width: 40rpx;height: 40rpx;margin-left: 20rpx;" @tap="scan"></image>
			</view>

			<view class="item">
				<text class="label">学生姓名</text>
				<input type="text" placeholder="请填写学生姓名" placeholder-style="font-size: 30rpx; color: #999" @input="getName" :disabled="isDisabled" :value="name" />
			</view>
			<view class="item">
				<text class="label">学生生日</text>
				<picker mode="date" :disabled="isDisabled" @change="bindDateChange" style="flex: 1;text-align: right;">
					<view style="font-size: 30rpx; color: #999">{{ birthDay }}</view>
				</picker>
			</view>
			<view class="item">
				<text class="label">学生性别</text>
				<picker mode="selector" @change="bindSexChange" style="flex: 1;text-align: right;" :range="sexArray" :value="sexIndex" :disabled="isDisabled">
					<view style="font-size: 30rpx; color: #999">{{ sexArray[sexIndex] }}</view>
				</picker>
			</view>
			<view class="item">
				<text class="label">家长姓名</text>
				<input type="text" placeholder="请填写家长姓名" placeholder-style="font-size: 30rpx; color: #999" @input="getParentName" :disabled="isDisabled" :value="parent_name" />
			</view>
			<view class="item">
				<text class="label">用户名</text>
				<input type="text" disabled value="已授权" />
			</view>
			<view class="item">
		...
		<view class="btn" v-if="teacherInfo && JSON.stringify(teacherInfo) != '{}'">
			<view @tap="applyForMod" v-if="change_class_status == 0"><text>申请修改</text></view>
			<view @tap="saveInfo" v-if="change_class_status == 1"><text>保存</text></view>
			<view @tap="applying" v-if="change_class_status == 2"><text>审批中</text></view>
		</view>
		<view v-else class="btn">
			<view @tap="saveInfo"><text>保存</text></view>
		</view>
		<view class="btn" v-if="from == 'home'">
			<view @tap="noBind" style="background: #ccc;"><text>暂不绑定</text></view>
		</view>
		<!-- 选择老师弹窗 -->

		<uni-popup ref="teacherChoose" :maskClick="false">
			<view class="popUp" :style="{ width: popUpWidth }">
				<view class="title"><text>请选择老师</text></view>
				<radio-group @change="radioChange">
					<label v-for="(item, index) in teacherList" :key="item.value">
						<view style="margin-right: 12rpx;"><radio :value="item.id" style="transform: scale(0.6);" color="#00B7CC" /></view>
						<view>{{ item.teacherInfo.true_name }}</view>
					</label>
				</radio-group>
				<view class="btn">
					<view @tap="chooseTeacher"><text>确定</text></view>
				</view>
			</view>
		</uni-popup>
	</view>
	...
	methods: {
			...
			if (tmpUserInfo && JSON.stringify(tmpUserInfo) != '{}' && tmpUserInfo.mobile && tmpUserInfo.mobile.replace(/\s*/g, '') != '') {
				this.$api
					...
					.then(res => {
						...
						// 初始信息
						this.schoolName = JSON.stringify(result.schoolInfo != '{}') ? result.schoolInfo.name : ''; //学校名称

						this.schoolId = JSON.stringify(result.schoolInfo != '{}') ? result.schoolInfo.id : ''; //学校id

						this.gradeName = JSON.stringify(result.gradeInfo != '{}') ? result.gradeInfo.name : ''; //年级

						this.gradeId = JSON.stringify(result.gradeInfo != '{}') ? result.gradeInfo.id : ''; //年级id

						this.className = JSON.stringify(result.childInfo) != '{}' ? result.childInfo.class : ''; //班级

						this.card_no = result.card_no ? result.card_no : ''; //卡号

						this.name = JSON.stringify(result.childInfo) != '{}' ? result.childInfo.name : ''; //学生姓名
						// 学生生日
						this.birthDay = JSON.stringify(result.childInfo) != '{}' ? result.childInfo.birth_day : '请选择学生生日';
						console.log(this.birthDay);

						this.parent_name = JSON.stringify(result.childInfo) != '{}' ? result.childInfo.parent_name : ''; //家长姓名

						this.sexIndex = JSON.stringify(result.childInfo) != '{}' ? result.childInfo.gender - 1 : 0; //学生性别
		...
		saveInfo() {
			...
			if (this.checkCard(this.card_no)) {
				...
				console.log(param);
				if (
					this.card_no.replace(/\s*/g, '') == '' ||
					this.name.replace(/\s*/g, '') == '' ||
					this.parent_name.replace(/\s*/g, '') == '' ||
					this.birthDay == '请选择学生生日' ||
					!this.gradeName ||
					!this.schoolName ||
					this.className == ''
				) {
